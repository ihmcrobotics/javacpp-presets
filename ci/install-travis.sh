...
if [[ "$OS" == "linux-x86" ]] || [[ "$OS" == "linux-x86_64" ]] || [[ "$OS" =~ android ]]; then
  ...
  if [[ "$PROJ" =~ spinnaker ]]; then
    if [ "$OS" == "linux-x86_64" ]; then
        if [[ $(find $HOME/downloads/spinnaker-3.1.0.79-amd64-pkg.tar.gz -type f -size +1000000c 2>/dev/null) ]]; then
          echo "Found spinnaker in cache and size seems ok"
        else
          echo "Downloading spinnaker as not found in cache or too small"
          python $TRAVIS_BUILD_DIR/ci/gDownload.py 1GikprQCTtJaq0q5I4UpWz0l1frp2AKOX $HOME/downloads/spinnaker-3.1.0.79-amd64-pkg.tar.gz
        fi
        tar xzvf $HOME/downloads/spinnaker-spinnaker-3.1.0.79-amd64-pkg.tar.gz -C $TRAVIS_BUILD_DIR/../
        ls $TRAVIS_BUILD_DIR/../spinnaker-3.1.0.79-amd64/*.deb | while read fName; do ar vx $fName; tar -xvf data.tar.xz; done;
        ln -s libSpinnaker_C.so.3.1.0.79 usr/lib/libSpinnaker_C.so.1
        ln -s libSpinnaker.so.3.1.0.79 usr/lib/libSpinnaker.so.1
        cp -a usr $TRAVIS_BUILD_DIR/../
        docker exec -ti $DOCKER_CONTAINER_ID /bin/bash -xec "cp -a $HOME/build/usr/* /usr/"
    fi
  fi
