...
if [[ "$PROJ" =~ spinnaker ]]; then
       ...
       if [ "$OS" == "windows-x86_64" ]; then
           if [[ $(find /c/Downloads/Spinnaker_3.1.0.79_*_v140_x64.msi -type f -size +1000000c 2>/dev/null) ]]; then
             echo "Found spinnaker in cache and size seems ok"
           else
             echo "Downloading spinnaker to cache as not found"
             /c/python27/python $APPVEYOR_BUILD_FOLDER/ci/gDownload.py 1jpX6bHXM6ivBOtTtlXFqQp0Hvi5x4NLc /c/Downloads/Spinnaker_3.1.0.79_Binaries_v140_x64.msi
             /c/python27/python $APPVEYOR_BUILD_FOLDER/ci/gDownload.py 1ishxJ4mmZiDO4NqnAGluBp24dvuzEf-z /c/Downloads/Spinnaker_3.1.0.79_SourceCode_v140_x64.msi
           fi
           # we can get these msi files by starting the installation from the exe file
           cmd.exe //c 'msiexec /quiet /i C:\Downloads\Spinnaker_3.1.0.79_Binaries_v140_x64.msi ADDLOCAL=ALL'
           cmd.exe //c 'msiexec /quiet /i C:\Downloads\Spinnaker_3.1.0.79_SourceCode_v140_x64.msi ADDLOCAL=ALL'
       elif [ "$OS" == "windows-x86" ]; then
           if [[ $(find /c/Downloads/Spinnaker_3.1.0.79_*_v140_x86.msi -type f -size +1000000c 2>/dev/null) ]]; then
             echo "Found spinnaker32 in cache and size seems ok"
           else
             echo "Downloading spinnaker32 to cache as not found"
             /c/python27/python $APPVEYOR_BUILD_FOLDER/ci/gDownload.py 1aPDomXsro4CmkvY-NV_GwkFb_fHkCfkO /c/Downloads/Spinnaker_3.1.0.79_Binaries_v140_x86.msi
             /c/python27/python $APPVEYOR_BUILD_FOLDER/ci/gDownload.py 1rG40RVJ8WN9sECSO8e3t9gin6YjEiPxV /c/Downloads/Spinnaker_3.1.0.79_SourceCode_v140_x86.msi
           fi
           # we can get these msi files by starting the installation from the exe file
           cmd.exe //c 'msiexec /quiet /i C:\Downloads\Spinnaker_3.1.0.79_Binaries_v140_x86.msi ADDLOCAL=ALL'
           cmd.exe //c 'msiexec /quiet /i C:\Downloads\Spinnaker_3.1.0.79_SourceCode_v140_x86.msi ADDLOCAL=ALL'
       fi
